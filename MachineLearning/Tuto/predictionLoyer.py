...
n = houseData.shape[0]
X = np.matrix([np.ones(n), houseData['surface'], houseData['arrondissement']]).T
y = np.matrix(houseData['price']).T
print(n,X.shape,y.shape)

plt.title("Prix en fonction se la surface et de l'arrondissement")
plt.xlabel("Surface")
plt.ylabel("Prix")
plt.scatter([X[:,1].T], [y.T], c=[X[:,2].T])
plt.show()
